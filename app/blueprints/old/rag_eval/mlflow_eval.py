...
from app.blueprints.old.rag_eval.dataset import DatasetVersion
...
class RagEvalMLflow:
    ...
    def eval_run(self, run_name: str, tags: dict | None = None):
        """Open an MLflow evaluation run under the current commit's parent run."""
    ...
    def run_evaluation(self, sample_size: int | None = None):
        """Evaluate retrieval quality. Call inside an active ``eval_run`` context."""
        parquet_path = (
    ...
    def predict_fn(self, question: str) -> dict:
        """Return the retrieval output in MLflow's RAG scorer format."""
